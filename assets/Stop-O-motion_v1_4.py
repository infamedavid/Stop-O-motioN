...
class SMQ_PT_Panel(bpy.types.Panel):
    ...
    def draw(self, context):
        ...
        layout.prop(context.scene, "smq_noise_scale")
        layout.prop(context.scene, "smq_noise_strength")
        layout.prop(context.scene, "smq_noise_depth")

        layout.prop(context.scene, "smq_noise_use_frame_range")

        row = layout.row(align=True)
        row.prop(context.scene, "smq_noise_frame_start")
        row.prop(context.scene, "smq_noise_frame_end")

        row = layout.row(align=True)
        row.prop(context.scene, "smq_noise_blend_in")
        row.prop(context.scene, "smq_noise_blend_out")

        layout.prop(context.scene, "smq_noise_influence")
        layout.operator("smq.update_noise_modifier", text="Update Modifier")
...
class SMQ_OT_AddNoiseModifier(bpy.types.Operator):
    ...
    def execute(self, context):
        ...
        self.report({'INFO'}, f"{added} noise modifier(s) added.")
        return {'FINISHED'}
...
class SMQ_OT_UpdateNoiseModifier(bpy.types.Operator):
    ...
    bl_label = "Update Noise Modifier"
    bl_options = {'REGISTER', 'UNDO'}
            ...
            if any(kp.select_control_point for kp in fc.keyframe_points):
                for mod in fc.modifiers:
                    if mod.type == 'NOISE':
                        ...
                        updated += 1
        if updated == 0:
            self.report({'WARNING'}, "No noise modifiers to update.")
            return {'CANCELLED'}
...
def register():
    ...
    bpy.types.Scene.smq_noise_scale = bpy.props.FloatProperty(name="Scale", default=20.0, min=5.0, max=30.0)
    bpy.types.Scene.smq_noise_strength = bpy.props.FloatProperty(name="Strength", default=0.007, min=0.001, max=0.02)
    bpy.types.Scene.smq_noise_depth = bpy.props.IntProperty(name="Depth", default=20, min=0, max=100)

    bpy.types.Scene.smq_noise_use_frame_range = bpy.props.BoolProperty(name="Frame Range", default=False)
    bpy.types.Scene.smq_noise_frame_start = bpy.props.IntProperty(name="Start", default=1)
    bpy.types.Scene.smq_noise_frame_end = bpy.props.IntProperty(name="End", default=250)
    bpy.types.Scene.smq_noise_blend_in = bpy.props.IntProperty(name="Blend In", default=0)
    bpy.types.Scene.smq_noise_blend_out = bpy.props.IntProperty(name="Blend Out", default=0)
    bpy.types.Scene.smq_noise_influence = bpy.props.FloatProperty(name="Influence", default=1.0, min=0.0, max=1.0)
...
def unregister():
    ...
    del bpy.types.Scene.smq_noise_scale
    del bpy.types.Scene.smq_noise_strength
    del bpy.types.Scene.smq_noise_depth
    del bpy.types.Scene.smq_noise_use_frame_range
    del bpy.types.Scene.smq_noise_frame_start
    del bpy.types.Scene.smq_noise_frame_end
    del bpy.types.Scene.smq_noise_blend_in
    del bpy.types.Scene.smq_noise_blend_out
    del bpy.types.Scene.smq_noise_influence
